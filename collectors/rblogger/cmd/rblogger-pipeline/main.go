...
func runPipeline(ctx context.Context, cfg Config, dryRun bool) (map[string]any, error) {
	...
	for _, hash := range newHashes {
		state.SeenURLHashes[hash] = true
	}
	state.UpdatedAt = formatClickHouseTime(doneAt)
...
func loadConfig(rebuildBoard bool) (Config, error) {
	...
	return Config{
		...
		StatePath:            envString("STATE_PATH", ".rblogger_state/seen_urls.json"),
...
func (p *KafkaPublisher) Validate(ctx context.Context) error {
	...
	dialer := &kafka.Dialer{ClientID: p.cfg.ClientID, Timeout: 10 * time.Second}
...
func (p *KafkaPublisher) writer() *kafka.Writer {
	...
	if strings.TrimSpace(p.cfg.ClientID) != "" || strings.TrimSpace(p.cfg.Username) != "" {
		transport := &kafka.Transport{ClientID: p.cfg.ClientID}
		if strings.TrimSpace(p.cfg.Username) != "" || strings.TrimSpace(p.cfg.Password) != "" {
...
func loadState(path string) State {
	state := State{SeenURLHashes: map[string]bool{}}
	payload, err := os.ReadFile(path)
	if err != nil {
		return state
	}
	if err := json.Unmarshal(payload, &state); err != nil {
		return State{SeenURLHashes: map[string]bool{}}
	}
	if state.SeenURLHashes == nil {
		state.SeenURLHashes = map[string]bool{}
	}
	return state
}
